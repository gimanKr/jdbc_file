...
//				dao로부터 반환받은 결과에 따라서 성공인지 실패인지 판단후 응답화면 결정(View 메소드 호출)
public class MemberController {
	...
	public void selectByUserId(String userId) {
		...
		if(m==null) {
			new MemberMenu().displayNoData(userId +"에 해당하는 고객을 찾을수 없습니다.");
		}else {
	 ...
	 * @param keyword : 사용자가 입력한 검색할 키워드면
	 */
	...
	public void deleteMember(String userId) {
		...
		if (result > 0) {
			new MemberMenu().displaySuccess(userId+"회원님이 삭제되었습니다.");
		}else {
			new MemberMenu().displayFail("삭제 실패하였습니다.");
		}
	}
